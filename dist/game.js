...
import { guests } from "./guests/guest.js";
...
async function startGame() {
    let rainbowTitle = chalkAnimation.rainbow(`Welcome to the game "Murder at the Mansion."`);
    await sleep();
    rainbowTitle.stop();
    ...
    const roomAnswers = await inquirer.prompt([
        {
            type: "list",
            name: "room",
            message: chalk.yellowBright("Select the room from which you want to pick up an item:"),
            choices: mansion.rooms,
        },
    ]);
    ...
    const itemAnswers = await inquirer.prompt([
        {
            type: "list",
            name: "item",
            message: chalk.yellowBright(`Select the item you want to pick up from ${room}:`),
            choices: clues,
        },
    ]);
    ...
    const roomAnswers = await inquirer.prompt([
        {
            type: "list",
            name: "room",
            message: chalk.yellowBright("Select the room from which you want to use an item:"),
            choices: mansion.rooms,
        },
    ]);
    const room = roomAnswers.room;
    ...
    const itemAnswers = await inquirer.prompt([
        {
            type: "list",
            name: "item",
            message: chalk.yellowBright(`Select the item you want to use from ${room}:`),
            choices: clues,
        },
    ]);
    const item = itemAnswers.item;
    if (room === "Library" && item === "A torn paper of a diary") {
        console.log(chalk.yellowBright(`Detective Sherlock uses ${item} and finds a clear poetry by "Tehzeeb Hafi:`));
        console.log(chalk.greenBright(`Parai Aag pe Roti Nahin Banaunga
        ...
        console.log(chalk.yellowBright(`Detective Sherlock uses ${item} but found no solid evidence that leads to the murderer's identification.`));
    }
};
...
const interrogateSuspect = async () => {
    ...
    if (selectedGuest) {
        console.log(chalk.yellowBright(`${detective.name} interrogates ${selectedGuest.name}.`));
        console.log(chalk.yellowBright(`Detective: "Is any of these items yours?"
        1-"A torn paper of a diary"
        2-"A bloody knife"
        3-"A suspicious note"
        4-A letter opener`));
        // Simulate the guest's response about the item
        console.log(chalk.yellowBright(`${selectedGuest.name}: "No, none of these items are mine, sir. I've never seen them before."`));
        // Now ask the guest to write two poetry lines
        console.log(chalk.yellowBright(`Detective: "Do you like poetry? Please write two lines for me."`));
        let poetry1, poetry2;
        ...
        if (selectedGuest.name === "Waqar" &&
            poetry1 === "Parai Aag pe Roti Nahin Banaunga" &&
            poetry2 === "Mai bheeg jaaunga chhatari nahin banaunga") {
            console.log(chalk.redBright(`${detective.name}: "That's the exact same lines from the torn paper of diary!"`));
            console.log(chalk.redBright(`${detective.name}: "Let me compare your handwriting with the torn paper."`));
            console.log(chalk.redBright(`${detective.name}: "I got you your writing is same as the "Torn piece of diary" contain i found in the library."`));
            console.log(chalk.redBright(`${detective.name}: "And its your bad luck you write the same line as the "Torn piece of diary have now come with me towards the jail."`));
